...
from app.scrapers.common import parse_bytes
...
def is_enabled(settings: Settings) -> bool:
    return bool(
        settings.nostradamus_private_key
        or settings.nostradamus_api_key
        or settings.nostradamus_private_ticket
    )
...
async def _wait_for_real_signin(page) -> None:
    ...
    raise ScrapingError("Nostradamus: anti-bot challenge did not clear")
...
async def _login_and_fetch(page, private_key: str) -> Dict[str, Any]:
    ...
    if "Making sure you&#39;re not a bot!" in html:
        raise ScrapingError("Nostradamus: blocked by anti-bot page after login")
    ...
    if (
        ...
        raise ScrapingError("Nostradamus: stats block not found on settings page")
...
async def get_stats(headless: bool = True) -> Dict[str, Any]:
    ...
    except (MissingCredentialsError, ScrapingError):
        raise
