...
USER_STATS_URL = "https://generation-free.org/api/user"


def is_enabled(settings: Settings) -> bool:
    return bool(settings.gfree_token)
...
async def get_stats(headless: bool = True) -> dict[str, Any]:
    ...
    token = get_settings().gfree_token
    if not token:
        raise MissingCredentialsError("Missing GFREE_TOKEN")
    ...
    try:
        ...
        raise ScrapingError(f"Failed to reach Generation-Free API: {exc}") from exc

    if response.status_code >= 400:
        raise ScrapingError(f"Failed to get Generation-Free stats: HTTP {response.status_code}")
