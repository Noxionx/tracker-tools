...
def is_enabled(settings: Settings) -> bool:
    return bool(settings.teamflix_token)
...
async def get_stats(headless: bool = True) -> dict[str, Any]:
    ...
    token = get_settings().teamflix_token
    if not token:
        raise MissingCredentialsError("Missing TEAMFLIX_TOKEN")
    ...
    try:
        ...
        raise ScrapingError(f"Failed to reach TeamFlix API: {exc}") from exc

    if response.status_code >= 400:
        raise ScrapingError(f"Failed to get TeamFlix stats: HTTP {response.status_code}")
